...
# @login_required
def home1(request):
    text = request.session.pop('recognized_text', "")
    print(text)
    post_detail=None
    getComment=None
    if not request.user.is_authenticated:
    ...
    return render(request, 'home_bootstrap.html', {'current_user':current_user,'profile_pic':profile_pic,'cover_image':cover_image,'designation':designation,'name':name,'getPost':getPost,'comments':getComment,'text':text})
...
def send_sms_notification(to, message):
    ...
    auth_token = 'c92b6ed83bca2d5baa5f4dba19cbb52a'
...
#
#         # Check if audio file is attached in the request
#         if 'audio' in request.FILES:
#             audio_file = request.FILES['audio']
#
#             # Use SpeechRecognition to recognize speech from audio file
...
#         else:
#             return JsonResponse({'error': 'No audio file attached in the request.'}, status=400)
#
    ...
    if request.method == 'POST':
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                text = recognizer.recognize_google(audio_data)
                print("You said:", text)
                request.session['recognized_text'] = text
            except sr.UnknownValueError:
                print("Sorry, I couldn't understand what you said.")
            except sr.RequestError as e:
                print("Could not request results; {0}".format(e))
    # return render(request,'home_bootstrap.html', {'text': text})
